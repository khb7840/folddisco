...
pub fn get_single_feature(
    ...
    match &hash_type {
        ...
        HashType::TertiaryInteraction => {
            if i == 0
                || j == 0
                || i == structure.num_residues - 1
                || j == structure.num_residues - 1
            {
                return false;
            }
        ...
        if has_feature {
            if let Some(multiple_bins) = multiple_bins {
                ...
                if nbin_dist == 0 || nbin_angle == 0 {
                    ...
                    let hash = GeometricHash::perfect_hash_as_u32(
                        &feature, hash_type, nbin_dist, nbin_angle,
                    );
                    hash_vec.push(hash);
                }
            }
...
impl HashType {
    pub fn amino_acid_index(&self) -> Option<Vec<usize>> {
        match self {
            ...
            | HashType::FolddiscoDist => Some(vec![0, 1]),
            _ => None,
        }
    }
    ...
    pub fn dist_index(&self) -> Option<Vec<usize>> {
        match self {
            ...
            | HashType::Hybrid
            | HashType::FolddiscoAngle
            | HashType::FolddiscoDist => Some(vec![2, 3]),
...
mod tests {
    ...
    fn test_get_geometric_hash_with_shifts() {
        ...
        if let Some(structure) = read_structure_from_path(structure_path) {
            ...
            println!(
                "Normal approach: {:?} ({} hashes)",
                duration_normal,
                hash_vec_normal.len()
            );
            println!(
                "Shifting approach: {:?} ({} hashes)",
                duration_shifts,
                hash_vec_with_shifts.len()
            );
            println!(
                "Performance ratio (shifts/normal): {:.2}x",
            ...
            let dedup_efficiency =
                100.0 * unique_hashes.len() as f64 / hash_vec_with_shifts.len() as f64;
            ...
            let time_per_hash_normal =
                duration_normal.as_nanos() as f64 / hash_vec_normal.len() as f64;
            let time_per_hash_shifts =
                duration_shifts.as_nanos() as f64 / hash_vec_with_shifts.len() as f64;

            println!("Time per hash (normal): {:.2} ns", time_per_hash_normal);
            println!("Time per hash (shifts): {:.2} ns", time_per_hash_shifts);
            println!(
                "Per-hash overhead: {:.2}x",
                time_per_hash_shifts / time_per_hash_normal
            );
        } else {
