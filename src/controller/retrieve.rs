...
use crate::structure::{
    coordinate::Coordinate, core::CompactStructure, kabsch::KabschSuperimposer,
};
...
pub fn retrieve_with_prefilter(
    ...
    let residue_types: Vec<u8> = compact
        .residue_name
        .iter()
        ...
        if is_feature {
            ...
            if let Some(multiple_bins) = multiple_bin {
                for (nb_d, nb_a) in multiple_bins.iter() {
                    let curr_hash = GeometricHash::perfect_hash(&feature, hash_type, *nb_d, *nb_a);
...
pub fn retrieval_wrapper_for_foldcompdb(
    db_key: usize,
    node_count: usize,
    query_vector: &Vec<GeometricHash>,
    _hash_type: HashType,
    _nbin_dist: usize,
    _nbin_angle: usize,
    multiple_bin: &Option<Vec<(usize, usize)>>,
    ...
    let compact = foldcomp_db_reader
        .read_single_structure_by_id(db_key)
        .expect("Error reading structure from foldcomp db");
    ...
    let candidate_pair_map: HashMap<usize, Vec<(usize, usize)>> =
        candidate_pairs
            .into_iter()
            .fold(HashMap::default(), |mut map, (qi, pair)| {
    ...
    let output: Vec<(
        ...
        .map(|component| {
            ...
            let query_to_retrieved: HashMap<usize, usize> = query_indices
                .iter()
            ...
            let mut retrieved_indices_scanned: Vec<usize> =
                Vec::with_capacity(retrieved_indices.len());
            ...
            let mut res_vec_from_hash: Vec<ResidueMatch> =
                Vec::with_capacity(all_query_indices.len());
            ...
            all_query_indices.iter().for_each(|&i| {
                ...
                if let Some(&retrieved_index) = query_to_retrieved.get(&i) {
                    ...
                    if !retrieved_indices_scanned_set.contains(&retrieved_index) {
                        ...
                        if let Some(prev_pos) = retrieved_indices_scanned
                            .iter()
                            .position(|&x| x == retrieved_index)
                        {
                    ...
                    if !count_map.is_empty() {
                        let max = count_map
                            .iter()
                            .filter(|&(_, &v)| v == *count_map.values().max().unwrap())
                            .map(|(&k, &v)| (k, v))
                            .collect::<Vec<_>>();

                        if max.len() == 1
                            && max[0].1 >= RESIDUE_RESCUE_COUNT_CUTOFF
                            && !retrieved_indices_scanned_set.contains(&max[0].0)
                        {
            ...
            let (rmsd, u_mat, t_mat, ca_coords, metrics) = if res_vec == res_vec_from_hash {
                (
                    ...
                    u_mat_from_hash,
                    t_mat_from_hash,
                    ca_coords_from_hash.clone(),
                    metrics_from_hash.clone(),
                )
            } else {
                rmsd_with_calpha_and_rottran(
                    query_structure,
                    &compact,
                    &query_indices_scanned,
                    &retrieved_indices_scanned,
                    partial_fit,
                )
            };
...
pub fn prefilter_amino_acid(
    query_set: &HashSet<GeometricHash>,
    _hash_type: HashType,
    compact: &CompactStructure,
    ...
    for edge in retrieved.edge_indices() {
        ...
        if let Some(&((query_i, query_j), _, _)) = query_map.get(&hash) {
            ...
            for (q, r) in pairs {
                ...
                if new_count > best_match[q].0
                    || (new_count == best_match[q].0 && r < best_match[q].1)
                {
...
pub fn rmsd_for_matched(
    compact1: &CompactStructure,
    compact2: &CompactStructure,
    index1: &Vec<usize>,
    index2: &Vec<usize>,
    lms: bool,
) -> f32 {
    let coord_vec1: Vec<Coordinate> = index1
        .iter()
        .map(|&i| {
            (
                compact1.ca_vector.get_coord(i).unwrap(),
                compact1.cb_vector.get_coord(i).unwrap(),
            )
        })
        .flat_map(|(a, b)| vec![a, b])
        .collect();

    let coord_vec2: Vec<Coordinate> = index2
        .iter()
        .map(|&i| {
            (
                compact2.ca_vector.get_coord(i).unwrap(),
                compact2.cb_vector.get_coord(i).unwrap(),
            )
        })
        .flat_map(|(a, b)| vec![a, b])
        .collect();
...
pub fn rmsd_with_calpha_and_rottran(
    ...
    let target_calpha: Vec<Coordinate> = index2
        .iter()
        .map(|&i| compact2.ca_vector.get_coord(i).unwrap())
        .collect();
    ...
    match lms {
        true => {
            if index1.len() <= 3 {
                ...
                let target_metrics =
                    match (&superposer.reference_coords, &superposer.transformed_coords) {
                        (Some(ref_coords), Some(trans_coords)) => {
                            let precomputed_distances =
                                PrecomputedDistances::new(&ref_coords, &trans_coords);
                ...
                (
                    superposer.get_rms(),
                    superposer.rot.unwrap(),
                    superposer.tran.unwrap(),
                    target_calpha,
                    target_metrics,
                )
            } else {
