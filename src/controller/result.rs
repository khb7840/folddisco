...
fn build_match_result_columns<'a>(qid: String, query_residues: String) -> HashMap<&'static str, Column<MatchResult<'a>>> {
    vec![
        ...
        Column::new("dms", "Distance Matrix Score (DMS)", |r: &MatchResult| Value::Float(r.metrics.dms, DEFAULT_FLOAT_PRECISION)),
        Column::new("pas", "Pseudo-Bond Angle Score (PAS)", |r: &MatchResult| Value::Float(r.metrics.pas, DEFAULT_FLOAT_PRECISION)),
        Column::new("sos", "Side-chain Orientation Score (SOS)", |r: &MatchResult| Value::Float(r.metrics.sos, DEFAULT_FLOAT_PRECISION)),
