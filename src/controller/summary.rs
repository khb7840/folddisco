...
use crate::utils::log::{log_msg, print_log_msg, FAIL, INFO};
...
impl EncodingStat {
    pub fn new(
        hash_type: HashType,
        dist_bin_given: usize,
        angle_bin_given: usize,
        ...
        let total_possible = total_possible_encodings(hash_type, dist_bin_given, angle_bin_given);
...
pub fn total_possible_encodings(
    hash_type: HashType,
    dist_bin_given: usize,
    angle_bin_given: usize,
) -> usize {
...
pub fn count_encodings(
    ...
    let mut stats = EncodingStat::new(hash_type, nbin_dist, nbin_angle, index);
    ...
    if verbose {
        print_log_msg(
            INFO,
            &format!(
                "Hash count vector loaded: {} entries",
                stats.hash_count_vec.len()
            ),
        );
    }
    ...
    stats.hash_count_vec.par_iter().for_each(|(hash, count)| {
        ...
        stats.aa_pair_counts.counts[aa1][aa2]
...
pub fn save_summary(
    ...
    if verbose {
        print_log_msg(
            INFO,
            &format!("Saving summary to files with prefix: {}", output_prefix),
        );
    }
    ...
    writeln!(
        f,
        "rank\thash\tcount\taa1\taa2\tca_dist\tcb_dist\tca_cb_angle\tphi1\tphi2"
    )?;
    ...
    if verbose {
        print_log_msg(
            INFO,
            &format!("Saved top {} encodings to {}", top_n, top_n_file),
        );
    }
    ...
    for aa1 in 0..20 {
        ...
        for aa2 in 0..20 {
            let count = encoding_stat.aa_pair_counts.counts[aa1 as usize][aa2 as usize]
    ...
    if verbose {
        print_log_msg(
            INFO,
            &format!("Saved amino acid pair counts to {}", aa_pairs_file),
        );
    }
    ...
    if verbose {
        print_log_msg(
            INFO,
            &format!("Saved count distribution to {}", count_dist_file),
        );
    }
...
pub fn analyze_enrichment(
    ...
    let mut hash_counts = hashes
        .iter()
        .fold(Vec::new(), |mut acc: Vec<(u32, usize)>, &hash| {
    ...
    let enriched_hashes = get_enriched_hashes(&hash_counts, &bg_hash_count_vec, p_value_threshold);
    ...
    for (hash, p_value) in enriched_hashes.iter() {
        ...
        if let Some(entry) = hash_id_pos_map.get(hash) {
            for (pdb_pos, pos1, pos2) in entry.iter() {
                let pos_str = format!("{}-{}-{}", query_handler.path_vec[*pdb_pos], pos1, pos2);
        ...
        writeln!(
            f,
            "{}\t{:.4e}\t{}\t{}",
            hash, p_value, features_str, positions_str
        )?;
    }
    ...
    pos_list.par_sort_by(|a, b| a.0 .0.cmp(&b.0 .0).then_with(|| b.1.len().cmp(&a.1.len())));
    ...
    for (pdb_pos, positions) in pdb_list.iter() {
        ...
        let mut filtered_with_counts: Vec<(String, usize)> = unique_positions
            .into_iter()
            .filter_map(|pos| {
                pos_count_map
                    .get(&(*pdb_pos, pos.clone()))
                    .map(|count| (pos, *count))
        ...
        let mut final_positions: Vec<String> = filtered_with_counts
            .iter()
            .map(|(pos, _)| pos.clone())
            .collect();
...
pub fn get_hash_count_vec(index: &FolddiscoIndex) -> Vec<(u32, usize)> {
...
fn get_enriched_hashes(
    query_hash_counts: &Vec<(u32, usize)>,
    bg_hash_counts: &Vec<(u32, usize)>,
    p_value_cutoff: f64,
) -> Vec<(u32, f64)> {
    ...
    let mut enriched: Vec<(u32, f64)> = query_hash_counts
        .par_iter()
...
fn hypergeometric_pmf(x: usize, n: usize, k: usize, n_total: usize) -> f64 {
    ...
    let log_prob = log_binomial(k, x) + log_binomial(n_total - k, n - x) - log_binomial(n_total, n);
        ...
        pool.install(|| {
            ...
            self.path_vec
                ...
                .for_each(|(pdb_pos, pdb_path)| {
                    ...
                    let compact = read_structure_from_path(pdb_path)
                        .expect(log_msg(FAIL, "Failed to read structure").as_str());
                    ...
                    res_bound.for_each(|(i, j)| {
                        ...
                        if has_feature {
                            if self.num_bin_dist == 0 || self.num_bin_angle == 0 {
                                let hash = GeometricHash::perfect_hash_default_as_u32(
                                    &feature,
                                    self.hash_type,
                                );
                                let pos1 = format!(
                                    "{}{}",
                                    compact.chain_per_residue[i] as char, compact.residue_serial[i]
                                );
                                let pos2 = format!(
                                    "{}{}",
                                    compact.chain_per_residue[j] as char, compact.residue_serial[j]
                                );
                                output_map
                                    .entry(hash)
                                    .or_default()
                                    .push((pdb_pos, pos1, pos2));
                            } else {
