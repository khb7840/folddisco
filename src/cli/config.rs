use crate::prelude::{log_msg, HashType, FAIL};
...
pub struct IndexConfig {
    ...
    pub indexing_cwd: Option<String>,
...
impl IndexConfig {
    pub fn new(
        ...
        chunk_size: usize,
        max_residue: usize,
        input_format: StructureFileFormat,
        foldcomp_db: Option<String>,
        indexing_cwd: Option<String>,
    ...
    pub fn from_toml(toml: &toml::Value) -> Self {
        ...
        let input_format =
            StructureFileFormat::get_with_string(toml["input_format"].as_str().unwrap());
        let foldcomp_db = toml
            .get("foldcomp_db")
            .map(|x| x.as_str().unwrap().to_string());
        let indexing_cwd = toml
            .get("indexing_cwd")
            .map(|x| x.as_str().unwrap().to_string());
        let multiple_bin = toml.get("multiple_bin").map(|x| {
            x.as_array()
                ...
                .map(|y| {
                    ...
                    (
                        bin[0].as_integer().unwrap() as usize,
                        bin[1].as_integer().unwrap() as usize,
                    )
                })
                .collect()
        });
    ...
    pub fn to_toml(&self) -> toml::Value {
        ...
        map.insert(
            "hash_type".to_string(),
            toml::Value::String(self.hash_type.to_string()),
        );
        map.insert(
            "num_bin_dist".to_string(),
            toml::Value::Integer(self.num_bin_dist as i64),
        );
        map.insert(
            "num_bin_angle".to_string(),
            toml::Value::Integer(self.num_bin_angle as i64),
        );
        map.insert(
            "grid_width".to_string(),
            toml::Value::Float(self.grid_width as f64),
        );
        map.insert(
            "chunk_size".to_string(),
            toml::Value::Integer(self.chunk_size as i64),
        );
        map.insert(
            "max_residue".to_string(),
            toml::Value::Integer(self.max_residue as i64),
        );
        map.insert(
            "input_format".to_string(),
            toml::Value::String(self.input_format.to_string()),
        );
        if let Some(foldcomp_db) = &self.foldcomp_db {
            map.insert(
                "foldcomp_db".to_string(),
                toml::Value::String(foldcomp_db.clone()),
            );
        }
        if let Some(indexing_cwd) = &self.indexing_cwd {
            map.insert(
                "indexing_cwd".to_string(),
                toml::Value::String(indexing_cwd.clone()),
            );
        }
        if let Some(multiple_bin) = &self.multiple_bin {
            map.insert(
                "multiple_bin".to_string(),
                toml::Value::Array(
                    multiple_bin
                        .iter()
                        .map(|x| {
                            toml::Value::Array(vec![
                                toml::Value::Integer(x.0 as i64),
                                toml::Value::Integer(x.1 as i64),
                            ])
                        })
                        .collect(),
                ),
...
pub fn write_index_config_to_file(path: &str, index_config: IndexConfig) {
    let mut file = std::fs::File::create(path).expect(&log_msg(
        FAIL,
        &format!("Unable to create config file: {}", path),
    ));
...
pub fn read_index_config_from_file(path: &str) -> IndexConfig {
    ...
    let file = std::fs::File::open(path)
        .expect(&log_msg(FAIL, &format!("Config file not found: {}", path)));
...
mod tests {
    ...
    fn test_write_index_config_to_file() {
        ...
        let index_config = IndexConfig::new(
            ...
            Some("data/foldcomp_db".to_string()),
            Some("/tmp".to_string()),
            Some(vec![(16, 4), (8, 3)]),
        );
