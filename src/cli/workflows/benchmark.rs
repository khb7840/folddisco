...
use crate::utils::benchmark::{
    compare_target_answer_neutral_vec, compare_target_answer_vec, measure_up_to_k_fp_vec,
    measure_up_to_k_fp_with_neutral_vec,
};
    ...
    match env {
        AppArgs::Benchmark {
            ...
            let input_vector = if input.is_none() {
                ...
                let file = std::fs::File::open(&input).expect(&log_msg(
                    FAIL,
            ...
            let mut lookup = raw_lookup
                .into_iter()
                .map(|(id, _, _, _, _)| parse_path(&id, afdb_to_uniprot).to_string())
                .collect::<Vec<_>>();
            lookup.sort();
            lookup.dedup();
...
fn read_one_column_as_vec(
    ...
    let file = std::fs::File::open(file).expect(&log_msg(
        FAIL,
        &format!("Failed to open tsv file: {}", file),
    ));
    let reader = std::io::BufReader::new(file);
...
fn parse_path(path: &str, afdb_to_uniprot: bool) -> &str {
    ...
    let path = if path.ends_with(".pdb")
        || path.ends_with(".cif")
        || path.ends_with(".fcz")
        || path.ends_with(".ent")
    {
        ...
        &path[..path.len() - 4]
    } else if path.ends_with(".pdb.gz")
        || path.ends_with(".cif.gz")
        || path.ends_with(".fcz.gz")
        || path.ends_with(".ent.gz")
    {
