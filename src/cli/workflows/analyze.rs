    ...
    match env {
        AppArgs::Analyze {
            ...
            if verbose {
                if compare_with_pdbs {
                    ...
                    print_log_msg(
                        INFO,
                        &format!(
                            "Summarizing encoding distribution of index {:?} using {} threads",
                            &index_path, threads
                        ),
                    );
                }
            ...
            rayon::ThreadPoolBuilder::new()
                .num_threads(threads)
                .build_global()
                .expect(&log_msg(FAIL, "Failed to build thread pool"));
            // Load index
            let (index, _offset_mmap) = measure_time!(load_folddisco_index(&index_path), verbose);
            ...
            if compare_with_pdbs {
                if let Some(pdb_container) = pdb_container {
                    ...
                    if saved_enrichment.is_err() {
                        print_log_msg(
                            FAIL,
                            &format!("Failed to analyze enrichment for PDBs in {}", pdb_container),
                        );
                    }
                } else {
                ...
                let summary = count_encodings(&index, hash_type, nbin_dist, nbin_angle, verbose);
                let saved = save_summary(&summary, &output_prefix, top_n, verbose);
                if saved.is_err() {
                    print_log_msg(
                        FAIL,
                        &format!("Failed to save summary result to {}", &output_prefix),
                    );
                }
            }
        }
