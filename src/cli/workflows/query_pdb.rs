...
fn resolve_target_structure_path(
    target_id: &str,
    index_prefix: &str,
    config: &IndexConfig,
) -> String {
    ...
    if let Some(indexing_cwd) = &config.indexing_cwd {
        let candidate = Path::new(indexing_cwd).join(target_id);
        if candidate.is_file() {
            return candidate.to_string_lossy().to_string();
        }
    }

    if let Some(index_dir) = Path::new(index_prefix).parent() {
        let candidate = index_dir.join(target_id);
        if candidate.is_file() {
            return candidate.to_string_lossy().to_string();
        }
    }
                    ...
                    if !skip_match {
                        measure_time!(
                            query_count_vec.par_iter_mut().for_each(|(_, v)| {
                                let resolved_tid =
                                    resolve_target_structure_path(v.tid, &index_prefix, &config);
                                #[cfg(not(feature = "foldcomp"))]
...
mod tests {
    ...
    #[test]
    fn test_resolve_target_structure_path_with_indexing_cwd() {
        let root = make_temp_dir("folddisco_path_resolve");
        let data_dir = root.join("data");
        fs::create_dir_all(&data_dir).unwrap();
        let pdb_path = data_dir.join("a.pdb");
        fs::write(&pdb_path, "ATOM\n").unwrap();

        let index_prefix = root.join("index").join("toy_idx");
        fs::create_dir_all(index_prefix.parent().unwrap()).unwrap();
        ...
        let config = IndexConfig::new(
            ...
            1,
            50000,
            crate::structure::io::StructureFileFormat::PDB,
            None,
            Some(root.to_string_lossy().to_string()),
            None,
        );
        let resolved =
            resolve_target_structure_path("data/a.pdb", index_prefix.to_str().unwrap(), &config);
        assert_eq!(resolved, pdb_path.to_string_lossy().to_string());

        fs::remove_dir_all(&root).unwrap();
    }

    #[test]
    fn test_resolve_target_structure_path_backward_compatible_fallback() {
        let root = make_temp_dir("folddisco_path_resolve_legacy");
        let index_prefix = root.join("index").join("toy_idx");
        fs::create_dir_all(index_prefix.parent().unwrap()).unwrap();

        let legacy_config = IndexConfig::new(
            HashType::PDBTrRosetta,
            16,
            4,
            20.0,
        ...
        let unresolved = resolve_target_structure_path(
            "legacy/rel/path.pdb",
            index_prefix.to_str().unwrap(),
            &legacy_config,
        );
        assert_eq!(unresolved, "legacy/rel/path.pdb".to_string());
