...
impl HashValue {
    ...
    pub fn perfect_hash(feature: &Vec<f32>, nbin_dist: usize, nbin_angle: usize) -> u32 {
        ...
        let ca_cb_angle = discretize_value(
            ...
            0.0,
            MAX_ANGLE_RAD,
            nbin_angle.min(NBIN_ANGLE_180),
        );

        // Two torsion angles:
        let phi1 = discretize_value(feature[5], MIN_ANGLE_RAD, MAX_ANGLE_RAD, nbin_angle);
        let phi2 = discretize_value(feature[6], MIN_ANGLE_RAD, MAX_ANGLE_RAD, nbin_angle);

        // Bit map: 9 for residue pairs, 3 for ca_dist, 3 for cb_dist, 5 for ca-cb angle, 5 for phi1, 5 for phi2
        let hashvalue =
            res_pair << 21 | ca_dist << 18 | cb_dist << 15 | ca_cb_angle << 10 | phi1 << 5 | phi2;
        hashvalue
    }
    ...
    pub fn reverse_hash(&self, nbin_dist: usize, nbin_angle: usize) -> [f32; 7] {
        ...
        let ca_cb_angle = continuize_value(
            ...
            0.0,
            MAX_ANGLE_RAD,
            (nbin_angle as f32).min(NBIN_ANGLE_180),
        );
