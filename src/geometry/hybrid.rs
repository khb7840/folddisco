...
impl HashValue {
    ...
    pub fn perfect_hash(feature: &Vec<f32>, nbin_dist: usize, nbin_angle: usize) -> u32 {
        ...
        let sin_bb_phi1 = discretize_value(sin_bb_phi1, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let cos_bb_phi1 = discretize_value(cos_bb_phi1, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let sin_bb_phi2 = discretize_value(sin_bb_phi2, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let cos_bb_phi2 = discretize_value(cos_bb_phi2, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);

        let hashvalue = res1_group << 30
            | res2_group << 28
            | ca_dist << 24
            | cb_dist << 20
            | sin_ca_cb_angle << 18
    ...
    pub fn perfect_hash_default(feature: &Vec<f32>) -> u32 {
        HashValue::perfect_hash(
            feature,
            HYBRID_NBIN_DIST as usize,
            HYBRID_NBIN_SIN_COS as usize,
        )
    }
...
mod tests {
    ...
    fn test_geometrichash_works() {
        ...
        let raw_feature2 = (
            b"VAL", b"PHE", 15.9_f32, 14.0_f32, 116.0_f32, 80.0_f32, -100.0_f32, -120.0_f32,
            -30.0_f32,
        );
        let raw_feature = vec![
            map_aa_to_u8_group(raw_feature.0) as f32,
            map_aa_to_u8_group(raw_feature.1) as f32,
            raw_feature.2,
            raw_feature.3,
            raw_feature.4.to_radians(),
            raw_feature.5.to_radians(),
            raw_feature.6.to_radians(),
            raw_feature.7.to_radians(),
            raw_feature.8.to_radians(),
        ];
        let raw_feature2 = vec![
            map_aa_to_u8_group(raw_feature2.0) as f32,
            map_aa_to_u8_group(raw_feature2.1) as f32,
            raw_feature2.2,
            raw_feature2.3,
            raw_feature2.4.to_radians(),
            raw_feature2.5.to_radians(),
            raw_feature2.6.to_radians(),
            raw_feature2.7.to_radians(),
            raw_feature2.8.to_radians(),
        ];
