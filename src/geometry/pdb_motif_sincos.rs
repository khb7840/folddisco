...
impl HashValue {
    ...
    pub fn perfect_hash(feature: &Vec<f32>, nbin_dist: usize, nbin_angle: usize) -> u32 {
        ...
        let sin_angle = discretize_value(sin_angle, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let cos_angle = discretize_value(cos_angle, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let hashvalue =
            res1 << 21 | res2 << 16 | ca_dist << 12 | cb_dist << 8 | sin_angle << 4 | cos_angle;
        hashvalue
    }
...
mod tests {
    ...
    fn test_geometrichash_works() {
        // Test perfect hash
        let raw_feature = (b"PHE", b"VAL", 14.0_f32, 15.9_f32, 116.0_f32);
        let raw_feature = vec![
        ...
        let hash: GeometricHash =
            GeometricHash::PDBMotifSinCos(HashValue(HashValue::perfect_hash(&raw_feature, 8, 3)));
        match hash {
