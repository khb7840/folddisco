...
impl HashValue {
    ...
    pub fn perfect_hash(feature: &Vec<f32>, nbin_dist: usize, nbin_angle: usize) -> u32 {
        ...
        let sin_ca_cb_angle =
            discretize_value(sin_ca_cb_angle, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let cos_ca_cb_angle =
            discretize_value(cos_ca_cb_angle, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        // Two torsion angles:
        let sin_phi1 = feature[5].sin();
        let cos_phi1 = feature[5].cos();
        let sin_phi2 = feature[6].sin();
        let cos_phi2 = feature[6].cos();
        let sin_phi1 = discretize_value(sin_phi1, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let cos_phi1 = discretize_value(cos_phi1, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let sin_phi2 = discretize_value(sin_phi2, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);
        let cos_phi2 = discretize_value(cos_phi2, MIN_SIN_COS, MAX_SIN_COS, nbin_angle);

        let hashvalue = res1 << 25
            | res2 << 20
            | ca_dist << 16
            | cb_dist << 12
            | sin_ca_cb_angle << 10
    ...
    pub fn perfect_hash_default(feature: &Vec<f32>) -> u32 {
        HashValue::perfect_hash(
            feature,
            PDBTR_NBIN_DIST as usize,
            PDBTR_NBIN_SIN_COS as usize,
        )
    }
    ...
    pub fn perfect_hash_with_shifts_dedup_inline(feature: &Vec<f32>) -> (u8, [u32; 8]) {
        ...
        let ca_dist_0 = Self::discretize_with_shift(feature[2], MIN_DIST, MAX_DIST, NBIN_DIST, 0.0);
        let cb_dist_0 = Self::discretize_with_shift(feature[3], MIN_DIST, MAX_DIST, NBIN_DIST, 0.0);
        ...
        let hashvalue = res1 << 25
            ...
            | ca_dist_0 << 16
            | cb_dist_0 << 12
            | sin_ca_cb_disc_pos << 10
            | cos_ca_cb_disc_pos << 8
            | sin_phi1_disc_pos << 6
            | cos_phi1_disc_pos << 4
            | sin_phi2_disc_pos << 2
            | cos_phi2_disc_pos;
        ...
        let hashvalue = res1 << 25
            ...
            | ca_dist_0 << 16
            | cb_dist_0 << 12
            | sin_ca_cb_disc_neg << 10
            | cos_ca_cb_disc_neg << 8
            | sin_phi1_disc_neg << 6
            | cos_phi1_disc_neg << 4
            | sin_phi2_disc_neg << 2
            | cos_phi2_disc_neg;
    ...
    pub fn perfect_hash_with_all_shifts_exhaustive(feature: &Vec<f32>) -> (usize, Vec<u32>) {
        ...
        let mut unique_hashes = Vec::with_capacity(
            unique_ca_dists.len()
                * unique_cb_dists.len()
                * unique_ca_cb_angles.len()
                * unique_phi1_angles.len()
    ...
    #[inline]
    pub fn perfect_hash_with_all_shifts_exhaustive_optimized(
        feature: &Vec<f32>,
    ) -> (usize, Vec<u32>) {
        const NBIN_DIST: f32 = PDBTR_NBIN_DIST;
        const NBIN_ANGLE: f32 = PDBTR_NBIN_SIN_COS;
    ...
    pub fn perfect_hash_with_max_32_shifts(feature: &Vec<f32>) -> (usize, Vec<u32>) {
        ...
        if ca_dist_pos != ca_dist_orig {
            ca_dist_values.push(ca_dist_pos);
        }
        if ca_dist_neg != ca_dist_orig && ca_dist_neg != ca_dist_pos {
            ca_dist_values.push(ca_dist_neg);
        }

        let mut cb_dist_values = vec![cb_dist_orig];
        if cb_dist_pos != cb_dist_orig {
            cb_dist_values.push(cb_dist_pos);
        }
        if cb_dist_neg != cb_dist_orig && cb_dist_neg != cb_dist_pos {
            cb_dist_values.push(cb_dist_neg);
        }
        ...
        let sin_ca_cb_disc_orig =
            discretize_value(sin_ca_cb_orig, MIN_SIN_COS, MAX_SIN_COS, NBIN_ANGLE);
        let cos_ca_cb_disc_orig =
            discretize_value(cos_ca_cb_orig, MIN_SIN_COS, MAX_SIN_COS, NBIN_ANGLE);
        let sin_phi1_disc_orig =
            discretize_value(sin_phi1_orig, MIN_SIN_COS, MAX_SIN_COS, NBIN_ANGLE);
        let cos_phi1_disc_orig =
            discretize_value(cos_phi1_orig, MIN_SIN_COS, MAX_SIN_COS, NBIN_ANGLE);
        let sin_phi2_disc_orig =
            discretize_value(sin_phi2_orig, MIN_SIN_COS, MAX_SIN_COS, NBIN_ANGLE);
        let cos_phi2_disc_orig =
            discretize_value(cos_phi2_orig, MIN_SIN_COS, MAX_SIN_COS, NBIN_ANGLE);
        ...
        let ca_cb_pos_changed =
            sin_ca_cb_disc_pos != sin_ca_cb_disc_orig || cos_ca_cb_disc_pos != cos_ca_cb_disc_orig;
        ...
        let ca_cb_neg_changed =
            sin_ca_cb_disc_neg != sin_ca_cb_disc_orig || cos_ca_cb_disc_neg != cos_ca_cb_disc_orig;
        if ca_cb_neg_changed
            && (sin_ca_cb_disc_neg != sin_ca_cb_disc_pos
                || cos_ca_cb_disc_neg != cos_ca_cb_disc_pos)
        {
        ...
        let phi1_pos_changed =
            sin_phi1_disc_pos != sin_phi1_disc_orig || cos_phi1_disc_pos != cos_phi1_disc_orig;
        if phi1_pos_changed {
        ...
        let phi1_neg_changed =
            sin_phi1_disc_neg != sin_phi1_disc_orig || cos_phi1_disc_neg != cos_phi1_disc_orig;
        if phi1_neg_changed
            && (sin_phi1_disc_neg != sin_phi1_disc_pos || cos_phi1_disc_neg != cos_phi1_disc_pos)
        {
        ...
        let phi2_pos_changed =
            sin_phi2_disc_pos != sin_phi2_disc_orig || cos_phi2_disc_pos != cos_phi2_disc_orig;
        if phi2_pos_changed {
        ...
        let phi2_neg_changed =
            sin_phi2_disc_neg != sin_phi2_disc_orig || cos_phi2_disc_neg != cos_phi2_disc_orig;
        if phi2_neg_changed
            && (sin_phi2_disc_neg != sin_phi2_disc_pos || cos_phi2_disc_neg != cos_phi2_disc_pos)
        {
        ...
        let max_combinations = ca_dist_values.len()
            * cb_dist_values.len()
            * ca_cb_angle_values.len()
            * phi1_angle_values.len()
            * phi2_angle_values.len();
...
mod tests {
    ...
    fn test_geometrichash_works() {
        ...
        let raw_feature = vec![
            map_aa_to_u8(raw_feature.0) as f32,
            map_aa_to_u8(raw_feature.1) as f32,
            raw_feature.2,
            raw_feature.3,
            raw_feature.4.to_radians(),
            raw_feature.5.to_radians(),
            raw_feature.6.to_radians(),
        ];
        let raw_feature2 = vec![
            map_aa_to_u8(raw_feature2.0) as f32,
            map_aa_to_u8(raw_feature2.1) as f32,
            raw_feature2.2,
            raw_feature2.3,
            raw_feature2.4.to_radians(),
            raw_feature2.5.to_radians(),
            raw_feature2.6.to_radians(),
        ];
    ...
    fn test_deduplication_inline_performance() {
        ...
        println!(
            "=== Deduplication Performance ({} iterations) ===",
            ITERATIONS
        );
        ...
        println!(
            "Inline dedup: {:.2}x",
            inline_per_iter as f64 / default_per_iter as f64
        );
        println!(
            "Exhaustive dedup: {:.2}x",
            exhaustive_per_iter as f64 / default_per_iter as f64
        );
        ...
        let (unique_count2, _) =
            HashValue::perfect_hash_with_shifts_dedup_inline(&duplicate_feature);

        let (unique_count3, _) =
            HashValue::perfect_hash_with_all_shifts_exhaustive(&duplicate_feature);

        println!("\nDeduplication effectiveness:");
        println!(
            "Diverse feature: {} unique out of 8 ({:.1}% efficiency)",
            unique_count1,
            unique_count1 as f64 / 8.0 * 100.0
        );
        println!(
            "Duplicate-prone feature: {} unique out of 8 ({:.1}% efficiency)",
            unique_count2,
            unique_count2 as f64 / 8.0 * 100.0
        );
        println!(
            "Duplicate-prone feature (exhaustive): {} unique out of 243 ({:.1}% efficiency)",
            unique_count3,
            unique_count3 as f64 / 243.0 * 100.0
        );
    }
    ...
    fn test_max_32_shifts_efficiency() {
        ...
        let boundary_feature = vec![
            ...
            17.0,
            8.499,
            12.3, // CA distance very close to bin boundary
            45.1_f32.to_radians(),
            119.9_f32.to_radians(),
            -60.1_f32.to_radians(),
        ];
        ...
        let (count_max32, hashes_max32) =
            HashValue::perfect_hash_with_max_32_shifts(&boundary_feature);
        let (count_exhaustive, hashes_exhaustive) =
            HashValue::perfect_hash_with_all_shifts_exhaustive(&boundary_feature);
        ...
        let (count_max32_centered, _) =
            HashValue::perfect_hash_with_max_32_shifts(&centered_feature);
        let (count_exhaustive_centered, _) =
            HashValue::perfect_hash_with_all_shifts_exhaustive(&centered_feature);

        println!("Hash generation comparison for centered feature:");
        println!(
            "  Max 32 shifts method: {} unique hashes",
            count_max32_centered
        );
        println!(
            "  Exhaustive method: {} unique hashes",
            count_exhaustive_centered
        );
        ...
        for &max32_hash in &hashes_max32 {
            if !hashes_exhaustive.contains(&max32_hash) {
                ...
                println!(
                    "  ERROR: Max32 hash {} not found in exhaustive!",
                    max32_hash
                );
            }
        }
        ...
        println!(
            "  Max 32: {:.1}",
            count_max32 as f64 / max32_per_iter as f64 * 1000.0
        );
        println!(
            "  Inline: {:.1}",
            count_inline as f64 / inline_per_iter as f64 * 1000.0
        );
        println!(
            "  Exhaustive: {:.1}",
            count_exhaustive as f64 / exhaustive_per_iter as f64 * 1000.0
        );
    ...
    fn test_feature_distribution_analysis() {
        ...
        let test_structures = vec!["data/AF-P17538-F1-model_v4.pdb"];
        ...
        println!(
            "Total features analyzed: {} from {} structures",
            all_features.len(),
            structures_analyzed
        );
        ...
        for (dim, name) in feature_names.iter().enumerate() {
            ...
            if dim < 2 {
                ...
                for &val in &values {
                    let bin =
                        ((val - MIN_DIST) / bin_width).min((current_nbin - 1) as f32) as usize;
                    bin_counts[bin] += 1;
                }
                ...
                for &val in &sin_values {
                    let bin =
                        ((val - MIN_SIN_COS) / bin_width).min((current_nbin - 1) as f32) as usize;
        ...
        let angles = vec![("CA-CB Angle", 2), ("Phi1 Torsion", 3), ("Phi2 Torsion", 4)];
    ...
    fn suggest_distance_binning(values: &[f32], name: &str) {
        ...
        println!(
            "    Current uniform: {:.2} Å bins, ~{} effective bins",
            current_bin_width, effective_bins
        );
    ...
    fn suggest_angle_binning(values: &[f32], name: &str) {
        ...
        println!(
            "    Current sin/cos bin width: {:.2}",
            current_sin_cos_bin_width
        );
